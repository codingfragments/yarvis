			...
			<ul class="flex flex-col gap-2">
				{#each cat.items as it}
					<li class="rounded-lg bg-base-100/30 px-3 py-2.5">
						...
						<p class="text-xs text-base-content/65 leading-snug break-words">{it.detail}</p>
						{#if it.relevance}<p class="text-xs text-primary/80 italic mt-1.5 break-words">↳ {it.relevance}</p>{/if}
					</li>
				{/each}
			</ul>
		</SectionCard>
