<script lang="ts">
  ...
  function extractHeadings() {
    ...
    const els = mainContent.querySelectorAll(
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    const dir = settings.current.briefings_dir;
    // Navigate relative to whichever file was last selected (side panel or main)
    const activeFile = store.sidePanelFile || store.currentFile;
    const fileIdx = store.files.findIndex(
...
<div class="flex h-[calc(100vh-3.5rem-1.75rem)] -mt-4 -mx-0">
  ...
  <BriefingSidebar
    ...
    {searchQuery}
    onDateChange={(key) =>
      store.selectDate(key, settings.current.briefings_dir)}
  ...
  <div class="flex-1 overflow-y-auto scroll-smooth" bind:this={mainContent}>
    ...
    {:else if store.error && !store.rawMarkdown}
      ...
      >
        ...
        <a href="/settings" class="btn btn-ghost btn-sm text-xs"
